...
func GetCoins(xmysql *mysql.XMySQL, useId int64) []types.CoinTypeReturn {
	coinsql := "select c.id id , url , coinname,sortname,tokenname,app_recharge,app_withdrawal,in_withdrawal,user_id,amount,frozen_amount from coin c  left JOIN (select * from user_amount where user_id = ?) ua on c.id = ua.type where c.status <> 0 ORDER BY chain_num asc"
	rows, err := xmysql.Query(coinsql, useId)
	if err != nil {
		fmt.Println("GetCoins", err)
		return nil
	...
	for rows.Next() {
		...
		_ = rows.Scan(&record.Id, &record.Url, &record.Coinname, &record.Sortname, &record.Tokenname, &record.AppRecharge, &record.AppWithdrawal, &record.InWithdrawal, &record.UserId, &record.Amount, &record.FrozenAmount)
		list = append(list, record)
	}
